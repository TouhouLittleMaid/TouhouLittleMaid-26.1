...
@MaidComponentDef("navigation")
public class MaidNavigationComponent implements MaidComponent, AiStepComponent {
    ...
    public MaidNavigationComponent(EntityMaid maid) {
        this.maid = maid;
        ...
        if (!level.isClientSide() && maid.isEffectiveAi()) {
            if (mode != Mode.WATER && maid.isInWater() && shouldStartOrStopSwim(5)) {
                ...
                if (switchToNavigation(Mode.WATER, waterNavigation)) {
                    host.swim.setWantToSwim(true);
                    host.swim.setReadyToLand(false);
                }
                    ...
                    if (!shouldStartOrStopSwim(2)) {
                        ...
                        if (!level.isWaterAt(endPos) && !level.isWaterAt(endPos.below())) {
                            ...
                            if (switchToNavigation(Mode.GROUND, basicNavigation)) {
                                host.swim.setReadyToLand(true);
                                host.swim.setWantToSwim(false);
                            }
                        ...
                        host.swim.setWantToSwim(true);
                        host.swim.setSwimTarget(endPos);
                    }
                } else if (endPos == null && host.swim.isGoingToBreath()) {
