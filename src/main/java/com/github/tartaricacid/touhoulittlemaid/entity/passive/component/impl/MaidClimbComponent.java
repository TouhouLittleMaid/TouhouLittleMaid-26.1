...
import com.github.tartaricacid.touhoulittlemaid.entity.passive.component.hook.ClimbHook;
...
@MaidComponentDef("climb")
public class MaidClimbComponent implements MaidComponent, BaseTickComponent, ClimbHook {
    ...
    public MaidClimbComponent(EntityMaid maid) {
        this.maid = maid;
    }
    ...
    public Set<Class<? extends MaidComponent>> dependsOn() {
        return Set.of(MaidSwimComponent.class);
    }
    ...
    @Override
    public Vec3 handleOnClimbable(Vec3 oriDelta) {
        // 主动爬行过程中严禁水平方向偏移，防止摔伤，y轴保持原样
        if (host.climb.isCanClimb()) {
     ...
     */
    @Override
    public boolean onClimbable(BooleanSupplier superMethod) {
    ...
    @Override
    public void travel(Vec3 travelVector, Consumer<Vec3> superMethod) {
        MaidSwimComponent swim = host.swim;
        if (maid.isInWater()) {
            if (swim.wantToSwim()) {
            ...
            } else if (swim.isReadyToLand() || maid.isUnderWater()) {
